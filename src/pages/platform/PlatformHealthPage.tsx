      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-white border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-500">Database Status</CardTitle>
          ...
          <Card className="bg-white border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900">{metrics.apiLatency}</div>
              <div className="text-xs text-slate-500 mt-1">Average Edge Function response</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900">{metrics.cpuUsage}</div>
              <div className="text-xs text-slate-500 mt-1">{metrics.memoryUsage} Memory</div>
            </CardContent>
          </Card>

          <Card className="bg-white border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-500">Global Cluster status</CardTitle>
              <ServerCrash className="h-4 w-4 text-emerald-500" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-emerald-400">100% Up</div>
              <div className="text-xs text-slate-500 mt-1">{metrics.status}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="bg-white border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
          <CardHeader className="flex flex-row items-center justify-between flex-wrap gap-2">
            <div>
              <CardTitle className="text-lg font-bold text-slate-900">Platform Diagnostic Node Checks</CardTitle>
              <p className="text-xs text-slate-500">Inspect server nodes, latency limits and run diagnostic probes</p>
            </div>
            <Button
          ...
          <CardContent className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500">Database Schema Replication Status</span>
                <Badge className="bg-emerald-500/15 text-emerald-400 border border-emerald-500/20">Synced</Badge>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500">Edge Functions Cache Hit Rate</span>
                <span className="text-slate-900 font-mono font-medium">94.2%</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500">Offline Queue Sync Load</span>
                <span className="text-slate-900 font-mono font-medium">Normal (0 items queued)</span>
              </div>
            </div>

            {/* Database Table stats */}
            <div>
              <p className="text-sm font-semibold text-slate-900 mb-2">Supabase Database Tables Metadata</p>
              <div className="overflow-auto rounded-xl border border-slate-200 bg-white">
                <Table>
                  <TableHeader className="border-b border-zinc-850">
                    <TableRow className="hover:bg-transparent border-b border-slate-200">
                      <TableHead className="text-slate-500 font-medium">Table Name</TableHead>
                      <TableHead className="text-slate-500 font-medium">Rows Enrolled</TableHead>
                      <TableHead className="text-slate-500 font-medium">Estimated Disk Size</TableHead>
                      <TableHead className="text-slate-500 font-medium">Index Status</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {dbTables.map((t) => (
                      <TableRow key={t.table} className="hover:bg-slate-50/20 border-b border-slate-200">
                        ...
                        <TableCell className="text-slate-700 font-mono">{t.rows.toLocaleString()}</TableCell>
                        <TableCell className="text-slate-500 font-mono text-xs">{t.size}</TableCell>
                        <TableCell>
