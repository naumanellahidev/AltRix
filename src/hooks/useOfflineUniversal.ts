    ...
    try {
      switch (item.type) {
        ...
        case 'message': {
          ...
          const { error: recError } = await api.from("admin_message_recipients").insert(recipients);
        ...
        case 'lead_update': {
          ...
          const { error } = await api.from("crm_leads").update(updates).eq("id", lead_id).eq("school_id", schoolId);
          if (error) throw error;
          break;
        }
