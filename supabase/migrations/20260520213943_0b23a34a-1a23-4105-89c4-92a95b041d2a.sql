...
BEGIN
  ...
  computed AS (
    SELECT e.student_id, e.first_name, e.last_name, e.class_section_id,
      e.class_name, e.section_name,
      a.attendance_rate, g.avg_grade_percentage, g.recent_grade_avg,
  ...
  SELECT
    ...
    COALESCE(c.attendance_rate, 100) AS attendance_rate,
    COALESCE(c.avg_grade_percentage, 0) AS avg_grade_percentage,
    COALESCE(c.recent_grade_avg, 0) AS recent_grade_avg,
    CASE
      WHEN c.att_count >= 3 AND c.attendance_rate < 80 THEN 'Low Attendance'
      WHEN c.grade_count >= 1 AND c.avg_grade_percentage < 70 THEN 'Low Grades'
  ...
  FROM computed c
  WHERE
    (c.att_count >= 3 AND c.attendance_rate < 80)
    OR (c.grade_count >= 1 AND c.avg_grade_percentage < 70)
    OR (c.grade_count >= 2 AND c.recent_count >= 2
        AND c.recent_grade_avg < c.avg_grade_percentage - 15)
  ORDER BY c.attendance_rate ASC NULLS LAST, c.avg_grade_percentage ASC;
END;
$function$;